...
class Cli extends AgentBase
{
    public function write($str, $serverInfo)
    {
        if ($this->request->getCommand() == 'default') {
            $this->writeToSocket(($serverInfo['step'] ? $serverInfo['step'] . ': ' : '') . $str);
        }
    }
    ...
    public function tick($serverInfo = [])
    {
        switch ($this->request->getCommand()) {
            case 'steps':
                ...
                if (!empty($serverInfo['step'])) {
                    $this->writeToSocket($serverInfo['step'] . ': execute');
                } else {
            ...
            case 'memory' :
                $this->writeToSocket(
                    $this->replaceOut("Memory usage: " . $this->convert(memory_get_peak_usage()))
                );
                break;
            case 'info':
    ...
    protected function getServerInfo($serverStatistic)
    {
        ...
        if ($serverStatistic['step'])
            $msg .= 'Current step: ' . $serverStatistic['step'] . "\r\n";
        ...
        $msg .= $separator;
        $msg .= "\r\nOS: " . php_uname();
        $msg .= "\r\nMax memory usage: " . ini_get('memory_limit') . "B";
        $msg .= "\r\nMemory usage: " . $this->convert(memory_get_peak_usage());
        $msg .= "\r\nGID: " . getmygid();
        $msg .= "\r\nPID: " . getmypid();
